...
class RotatorController:
    ...
    """
    def __init__(self,port: str = portname, baud=115200, wait_time=1,loclon=lon,loclat=lat,locheight=height,utcoffset=timezone):
        """
    ...
    def get_loc(self):
        ...
        while i < 5:
            self.response = self.get_response()
            self.az = self.response[1]*100+self.response[2]*10 + self.response[3] + self.response[4]/10 - 360.0
            self.el = self.response[6]*100+self.response[7]*10 + self.response[8] + self.response[9]/10 - 360.0
            time.sleep(0.1/5)
            i += 1
    ...
    def get_loc_100(self):
        ...
        while i < 5:
            self.response_100 = self.get_response_100()
            self.az_100 = self.response_100[1]*100+self.response_100[2]*10 + self.response_100[3] + self.response_100[4]/10 + self.response_100[5]/100 - 360.0
            self.el_100 = self.response_100[6]*100+self.response_100[7]*10 + self.response_100[8] + self.response_100[9]/10 + self.response_100[10]/100 - 360.0
            time.sleep(0.1/5)
            i += 1
        print(f"rotator current position is AZ = {self.az_100}, EL = {self.el_100}.")
        return self.az_100, self.el_100
